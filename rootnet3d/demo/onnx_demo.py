...
def generate_patch_image(cvimg, bbox):
    ...
    src = np.array([[bb_c_x, bb_c_y], [bb_width, bb_c_y], [bb_c_x, bb_height]], dtype=np.float32)
    dst = np.array([[half_length, half_length], [input_length, half_length], [half_length, input_length]], dtype=np.float32)
...
class RootNet(object):
    ...
    def inference_single_person(self, crop_img, k_value):
        crop_img = transform(crop_img).astype(np.float32)
        k_value = [[[k_value]]]
    ...
    def inference(self, img: np.ndarray, bboxes):
        ...
        for i in range(num_person):
            ...
            crop_img, trans = generate_patch_image(img, bbox)
            k_value = np.sqrt(self.k_divisor / (bbox[2]*bbox[3]))
            
            root_3d = self.inference_single_person(crop_img, k_value) 
            return root_3d
